...
class MutationRateLoss(RegressionLossFunction):
    ...
    def __call__(self, y, raw_predictions, sample_weight=None):
        ...
        objective =  -np.sum(y) * np.log( np.sum( sample_weight * np.exp(1/2*raw_predictions) ) ) \
            + 1/2*np.sum(np.log(raw_predictions))
    ...
    def negative_gradient(self, y, raw_predictions, **kargs):
        ...
        grads = -np.sum(y)/np.sum(y_hat) * y_hat * 1/2 + 1/(2*raw_predictions)
...
def optim_tree(learning_rate = 1.,*,
    ...
    exposures = np.vstack([expos * np.exp(1/2*(lognu**2)) 
