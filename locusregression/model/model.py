...
class LocusRegressor:
    ...
    def get_component_locus_distribution(self, corpus_name):
        ...
        '''
        return self.corpus_states[corpus_name].log_mutation_rate
    ...
    def assign_sample_to_corpus(self, sample,
        ...
        posterior_likelihood = np.exp(logp_corpus - logsumexp(logp_corpus))

        return {
            'corpus' : list(self.corpus_states.keys()),
            'log_marginal_likelihood' : list(logp_corpus),
            'posterior_likelihood' : list(posterior_likelihood),
            'variance_AIS' : [np.std(w) for w in corpus_weights]
        }
